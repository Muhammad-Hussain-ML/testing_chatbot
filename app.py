...
def chat_interface():
    ...
    if query := st.chat_input("Ask a query..."):
        ...
        # --- START OF THE FIX ---
        # Format the chat history from a list of dicts to a list of strings
        # This matches the `history: List[str]` requirement of your API
        formatted_history = [f"{msg['role']}: {msg['content']}" for msg in st.session_state["messages"]]

        # Prepare data for the API request with the correctly formatted history
        data = {
            "query": query,
            "unique_id": unique_id,
